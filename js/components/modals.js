  ...
  var Modals = {
    addTransaction: function () {
      ...
      var goalsOptionsHtml = goals.map(function (g) {
        return `<option value="goal_${g.id}">🎯 Savings Goal: ${g.name}</option>`;
      }).join('');

      var buckets = DataStore.getMoneyJobs() || [];
      var bucketsOptionsHtml = buckets.map(function (b) {
        var remaining = b.assigned - (b.spent || 0);
        return `<option value="${b.id}">${b.name} (${SavvySpend.formatCurrencyPlain(remaining)})</option>`;
      }).join('');
        ...
        <form id="txn-form" class="mt-md" style="max-height: 70vh; overflow-y: auto; padding-right: 4px;">
          ...
          <div class="form-group mt-md" id="group-rating">
            ...
            <div class="flex gap-sm" style="width: 100%;">
              <button type="button" class="btn btn-outline btn-sm rate-chip active" data-rating="worth_it" style="flex: 1; text-align: center; font-size: 0.8rem; height: 38px;">😊 Worth It</button>
              <button type="button" class="btn btn-outline btn-sm rate-chip" data-rating="neutral" style="flex: 1; text-align: center; font-size: 0.8rem; height: 38px;">😐 Neutral</button>
              <button type="button" class="btn btn-outline btn-sm rate-chip" data-rating="regret" style="flex: 1; text-align: center; font-size: 0.8rem; height: 38px;">😞 Regret</button>
            </div>
    ...
    addBudget: function () {
      ...
      document.getElementById('budget-form').addEventListener('submit', function (e) {
        ...
        var catDetails = categories[category];
    ...
    editBudget: function (id) {
      ...
      var html = `
        <div class="modal-header flex flex-between">
          <h3 class="modal-title">Edit ${budget.name} Budget</h3>
          <button class="btn-icon" onclick="SavvySpend.closeModal()"><i data-lucide="x"></i></button>
        </div>
      ...
      var html = `
        ...
        <form id="goal-form" class="mt-md">
          ...
          <div class="form-group mt-md">
            ...
            <div class="icon-selector-grid" style="display: grid; grid-template-columns: repeat(6, 1fr); gap: 12px; margin-top: 8px;">
              <button type="button" class="btn-icon-select active" data-icon="target" style="padding: 10px; border: 2px solid var(--primary); border-radius: var(--radius-md); background: var(--bg-hover); display: flex; align-items: center; justify-content: center; cursor: pointer; color: var(--primary);">
                <i data-lucide="target"></i>
              </button>
              <button type="button" class="btn-icon-select" data-icon="plane" style="padding: 10px; border: 2px solid var(--border); border-radius: var(--radius-md); background: transparent; display: flex; align-items: center; justify-content: center; cursor: pointer; color: var(--text-secondary);">
                <i data-lucide="plane"></i>
              </button>
              <button type="button" class="btn-icon-select" data-icon="shield" style="padding: 10px; border: 2px solid var(--border); border-radius: var(--radius-md); background: transparent; display: flex; align-items: center; justify-content: center; cursor: pointer; color: var(--text-secondary);">
                <i data-lucide="shield"></i>
              </button>
              <button type="button" class="btn-icon-select" data-icon="laptop" style="padding: 10px; border: 2px solid var(--border); border-radius: var(--radius-md); background: transparent; display: flex; align-items: center; justify-content: center; cursor: pointer; color: var(--text-secondary);">
                <i data-lucide="laptop"></i>
              </button>
              <button type="button" class="btn-icon-select" data-icon="car" style="padding: 10px; border: 2px solid var(--border); border-radius: var(--radius-md); background: transparent; display: flex; align-items: center; justify-content: center; cursor: pointer; color: var(--text-secondary);">
                <i data-lucide="car"></i>
              </button>
              <button type="button" class="btn-icon-select" data-icon="home" style="padding: 10px; border: 2px solid var(--border); border-radius: var(--radius-md); background: transparent; display: flex; align-items: center; justify-content: center; cursor: pointer; color: var(--text-secondary);">
    ...
    editProfile: function () {
      ...
      if (user.avatarUrl) {
        ...
        for (var i = 0; i < colors.length; i++) {
          var c = colors[i];
          if (urlLower.indexOf('background=' + c.toLowerCase()) !== -1) {
            activeColor = c;
            break;
          }
        }
    ...
    addCustomCategory: function (catId) {
      ...
      var iconGridHtml = icons.map(function (ico) {
        ...
        return `
          <button type="button" class="btn-icon-select ${isSel}" data-icon="${ico}" style="width: 40px; height: 40px; border-radius: var(--radius-sm); cursor: pointer; display: flex; align-items: center; justify-content: center; outline: none; background: var(--bg-card); ${border}">
            <i data-lucide="${ico}" style="width: 18px; height: 18px; color: var(--text-primary);"></i>
          </button>
        `;
      }).join('');
      ...
      var colorGridHtml = colors.map(function (col) {
        ...
        return `
          <button type="button" class="btn-color-select ${isSel}" data-color="${col}" style="width: 32px; height: 32px; border-radius: 50%; background: ${col}; cursor: pointer; outline: none; ${border}"></button>
        `;
      }).join('');
      ...
      var html = `
        ...
        <form id="cat-form" class="mt-md">
          <div class="form-group">
            ...
            <input class="form-input" type="text" id="cat-name" value="${name}" placeholder="e.g. Subscriptions, Hobbies" required style="height: 44px;">
          </div>
          ...
          <div class="form-group mt-md">
            ...
            <input type="hidden" id="cat-icon" value="${activeIcon}">
          </div>
          ...
          <div class="form-group mt-md">
            ...
            <input type="hidden" id="cat-color" value="${activeColor}">
          </div>
    ...
    addFutureNote: function (noteId) {
      ...
      var catOptions = categories.map(function (c) {
        return `<option value="${c.id}" ${c.id === activeCatId ? 'selected' : ''}>${c.name}</option>`;
      }).join('');
    ...
    addMoneyJob: function (jobId) {
      ...
      var iconGridHtml = icons.map(function (ico) {
        ...
        var border = isSel ? 'border: 2px solid var(--primary); background: var(--bg-secondary);' : 'border: 1px solid var(--border);';
        return `
          <button type="button" class="btn-job-icon-select ${isSel}" data-icon="${ico}" style="width: 40px; height: 40px; border-radius: var(--radius-sm); cursor: pointer; display: flex; align-items: center; justify-content: center; outline: none; background: var(--bg-card); ${border}">
            <i data-lucide="${ico}" style="width: 18px; height: 18px; color: var(--text-primary);"></i>
          </button>
        `;
      }).join('');
      ...
      var colorGridHtml = colors.map(function (col) {
        ...
        var border = isSel ? 'border: 3px solid var(--text-primary);' : 'border: 1px solid transparent;';
        return `
          <button type="button" class="btn-job-color-select ${isSel}" data-color="${col}" style="width: 32px; height: 32px; border-radius: 50%; background: ${col}; cursor: pointer; outline: none; ${border}"></button>
        `;
      }).join('');
    ...
    editProfile: function () {
      ...
      var colorGridHtml = colors.map(function (c) {
        ...
        var border = isSel ? 'border: 3px solid var(--text-primary);' : 'border: 1px solid transparent;';
        return `
          <button type="button" class="btn-avatar-color-select ${isSel}" data-color="${c.hex}" style="width: 36px; height: 36px; border-radius: 50%; background: #${c.hex}; cursor: pointer; outline: none; ${border}"></button>
        `;
      }).join('');
      ...
      var html = `
        ...
        <form id="edit-profile-form" class="mt-md">
          <div class="form-group">
            ...
            <input class="form-input" type="text" id="ep-name" value="${user.name}" placeholder="Full Name" required style="height: 44px;">
          </div>

          <div class="form-group mt-md">
            <label class="form-label" for="ep-email">Email Address</label>
            <input class="form-input" type="email" id="ep-email" value="${user.email}" placeholder="Email Address" required style="height: 44px;">
          </div>
          ...
          <div class="form-group mt-md">
            ...
            <input type="hidden" id="ep-avatar-bg" value="${currentBg}">
          </div>
